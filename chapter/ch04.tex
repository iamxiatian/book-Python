...
\index{类型提示}类型提示（type hinting）是构建大型可维护系统的重要保障。本章将探讨现代Python类型系统的完整技术栈，构建从开发时到运行时的全方位类型安全保障。内容包括：静态类型系统，涵盖泛型、协议等高级类型提示技巧；类型检查工具，通过静态分析在编码阶段提前发现潜在错误；以及如何将类型系统延伸至运行时验证，利用Pydantic等工具确保外部输入数据的类型合规性，最终实现端到端的类型安全。
...
\index{Pydantic}Pydantic是一个基于Python类型提示的现代化数据验证库，它能够在运行时对数据进行验证和序列化操作。Pydantic的核心优势在于直接复用Python的类型提示语法，实现了静态类型系统与运行时验证的无缝衔接。其基础构建块是\inlinepython{BaseModel}类，通过继承此类并定义类型注解字段，即可获得自动的数据验证和转换能力。
