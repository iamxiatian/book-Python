...
类型提示（Type Hinting）是构建大型可维护系统的重要保障。本章将探讨现代Python类型系统的完整技术栈，构建从开发时到运行时的全方位类型安全保障。内容包括：静态类型系统，涵盖泛型、协议等高级类型提示技巧；类型检查工具，通过静态分析在编码阶段提前发现潜在错误；以及如何将类型系统延伸至运行时验证，利用Pydantic等工具确保外部输入数据的类型合规性，最终实现端到端的类型安全。
...
Python解释器在运行时并不会强制检查所有类型错误，专业的类型提示需要配合专门的类型检查工具才能真正发挥作用。在目前 Python 生态中，mypy\footnote{\url{https://mypy.readthedocs.io/}} 和 pyright\footnote{\url{https://github.com/microsoft/pyright}} 是最主流的两个类型检查器，下面简要说明它们的用法。

\subsection{Python社区的类型检查基石：Mypy}

Python类型检查生态的先驱当属Mypy无疑，它基于 Python 的类型提示标准，提供了严格的类型验证能力，并拥有最广泛的社区采用度和丰富的配置选项。

\heading{安装与使用}

安装Mypy十分简单，如果已经熟悉 uv 工具，可以直接使用以下命令：
...
对于采用\inlinefile{pyproject.toml}管理的项目，推荐在的\inlinepython{[tool.mypy]} 节中进行配置。例如，Mypy对于未指定返回类型的函数，默认不会给出类型检查报错信息，但通过指定\mintinline{toml}{disallow_untyped_defs = true}，可以强制要求函数必须指定返回类型。

以下列出若干关键配置选项及其作用：

\begin{minted}{toml}
# pyproject.toml 中的 mypy 配置节
[tool.mypy]
...
由微软开发的Pyright以出色的性能表现和精确的类型推断能力而著称，也是Pylance语言服务器的基础。
Pylance同样由微软开发，基于Pyright类型检查器构建，结合静态类型检查、智能代码补全和丰富的语言特性，为VSCode提供强大的Python开发支持，用户可通过VSCode的扩展商店安装Pylance。
...
类型提示的价值不仅限于静态检查阶段。通过Pydantic库\footnote{\url{https://docs.pydantic.dev/}}，我们能够在运行时将类型声明转化为实际的数据验证逻辑，实现类型驱动开发模式。这种模式让类型注解从开发时的文档约束，升级为运行时的安全保障。

\subsection{Pydantic 的核心价值与 BaseModel}

Pydantic是一个基于Python类型提示的现代化数据验证库，它能够在运行时对数据进行验证和序列化操作。Pydantic的核心优势在于直接复用Python的类型提示语法，实现了静态类型系统与运行时验证的无缝衔接。其基础构建块是\inlinepython{BaseModel}类，通过继承此类并定义类型注解字段，即可获得自动的数据验证和转换能力。
...
除了基础类型验证，Pydantic提供了丰富的验证器来支持复杂的业务规则验证：
...
FastAPI 是一个现代化的高性能 Python Web 框架，专门用于构建 API 接口。它深度集成了 Pydantic，能够自动利用类型注解进行请求和响应数据的验证，为 Web 应用开发提供强大的类型安全保障。
...
将上面的代码保存为 \inlinefile{demo_pydantic.py}，然后启动服务：

\begin{minted}{bash}
# 启动开发服务器
uv run uvicorn demo_pydantic:app --reload
\end{minted}
...
\begin{minted}{json}
{
    ...
    "username": "小白",
    "email": "xiaobai@example.com",
    "created_at": "2025-11-30T13:27:27.002075" # 运行时的系统时间
}
\end{minted}
