...
随着系统复杂度的提高，开发者面临着如何快速定位问题、理解系统运行状态以及保障系统稳定性的挑战。本章将探讨Python应用的可观测性体系，内容侧重于开发者日常工作中最直接相关的日志记录与交互式调试技术，并对指标监控与分布式追踪的基本思想作初步阐述，帮助读者逐步建立系统的可观测能力。

\section{可观测性基础}

开发者常依赖简单的print语句来理解程序行为，但随着系统规模扩大，这种方式已远不能满足需求。\index{可观测性}\textbf{可观测性（observability）}\citep{observability2025}应运而生，它指的是通过系统外部输出来推断其内部状态的能力。与传统的监控相比，可观测性更强调从系统外部视角主动发现问题，而非被动地等待警报。
...
可观测性体系包含如图\ref{fig:observability}所示的三大支柱，具体如下：

\begin{itemize}
    \item \textbf{日志（Logs）}： \index{日志}\index{Logs}记录离散事件，描述``在某个时间点，系统发生了什么事情''。日志通常是结构化的文本或二进制记录，包含时间戳、日志级别、事件描述、上下文信息等。例如用户登录、数据库查询失败、异常堆栈信息等。日志是事后分析的主要依据，但通常不具备实时性。
    \item \textbf{指标（Metrics）}： \index{指标}\index{Metrics}表示聚合的、可量化的数据，用于衡量``系统运行得如何''。指标通常是数值型的，如CPU使用率、请求响应时间、错误计数等。指标支持实时聚合和可视化，适合用于监控面板、警报和趋势分析。
    \item \textbf{追踪（Traces）}：\index{追踪}\index{Traces}记录单个请求在分布式系统中多个服务间的流转路径，描述``一个操作是如何发生的''。追踪通过唯一的Trace ID将跨服务边界的多个操作串联起来，形成调用链（call chain），常用于性能瓶颈分析和请求生命周期追踪。
\end{itemize}

总体而言，日志提供事件详情，指标提供系统健康状态，追踪提供请求执行路径。\index{遥测数据}\index{telemetry data}这三类数据在可观测性领域被统称为遥测数据（telemetry data）。


\section{日志记录}

在Python应用开发中，日志记录不仅是简单的输出工具，更是系统可观测性的基础组件。与简单的print语句相比，专业的日志系统提供了结构化输出、分级控制、环境感知等关键能力。本节将系统介绍Python的三种主流日志方案：标准库logging模块、第三方Loguru库以及专为结构化日志设计的structlog库，帮助读者根据项目需求选择合适的工具。
...
\index{logging}Python的logging模块采用了职责分离的模块化设计，构建了一个由四个核心组件组成的灵活体系：Logger（记录器）、Handler（处理器）、Formatter（格式化器）和Filter（过滤器）。这些组件通过一个标准化的数据载体LogRecord（日志记录对象）协同工作，形成完整的日志处理流水线。
...
\index{日志级别}logging模块定义了5个标准日志级别，每个级别都有明确的语义和使用场景。表\ref{tab:log:levels}展示了这些级别及其对应的数值和常见使用场景。
...
\subsection{Loguru：简洁易用的日志库}

\index{Loguru}Loguru是一款专为简化Python日志记录流程设计的第三方库，以开箱即用为核心特点，通过优雅的API设计和约定优于配置的理念，降低日志配置的复杂度。相比标准库logging需手动构建Logger、Handler、Formatter等组件，Loguru摒弃了繁琐的配置流程，提供零配置启动的便捷体验，同时原生支持日志着色、文件轮转、异常追溯等实用功能，在不牺牲核心灵活性的前提下，大幅降低了使用门槛。
...
使用Loguru时，无需显式获取和命名\inlinepython{logger}，只需\inlinepython{from loguru import logger}。每次使用这个导入的\inlinepython{logger}时，都会创建一个记录，并自动包含上下文\inlinepython{\_\_name\_\_}值，其他使用方式则与标准的logging相似。
...
\subsection{structlog：结构化日志库}

\index{结构化日志}\index{structlog}structlog是一款专注于结构化日志记录的第三方库，其核心理念是将日志视作数据结构而非纯文本。相较于Python标准库logging，它提供了更灵活、强大的日志处理能力，特别适合需要结构化输出、复杂上下文管理和高性能日志处理的现代应用场景。凭借处理器管道的设计模式，每条日志记录都会依次经过事件绑定、处理器转换和格式化输出的完整流程，这也让structlog能够轻松实现结构化日志、上下文自动注入等高级功能。
...
structlog的上述特性使其尤其适合需要高级日志功能的应用开发。在微服务架构、云原生部署等场景中，其结构化输出、上下文绑定和灵活配置等优势，能显著提升系统的可观测性与运维效率。
...
Python提供了多种交互式调试工具，从标准库内置的pdb调试器到breakpoint()函数，再到功能增强的第三方调试工具，形成了完整的调试工具链。

\subsection{标准pdb调试器}

\index{pdb}Python标准库自带的pdb（Python Debugger）调试器采用了命令行交互模式，不仅提供了无需额外依赖的可靠调试能力，还能帮助开发者深入理解代码的执行机制。在图形界面不可用或IDE支持受限的场景中，pdb更是进行问题排查的关键工具。

\heading{pdb调试器使用方法}

以下示例展示了pdb调试器的基本使用方法：
...
pdb还支持许多高级功能，包括条件断点、断点命令列表、临时断点、异常断点等。更多信息可参考官方文档：\url{https://docs.python.org/3/library/pdb.html}。
...
\index{ipdb}ipdb（IPython-enabled pdb）是pdb的增强版，提供了IPython的增强功能，如自动补全、语法高亮、更好的回溯显示等。ipdb的使用方式与pdb相同，在安装了ipdb之后，可以在代码中添加\inlinepython{ipdb.set\_trace()}来暂停执行并进入交互式调试模式，或者通过\inlinepython{import ipdb as pdb}来导入pdb，如上文pdb示例代码中注释掉的代码行所示。
...
如图\ref{fig:pudb}所示，\index{PuDB}PuDB是一个基于控制台的全屏可视化调试器，它把现代图形界面调试器的调试功能，打包进一个更轻量、且全程可用键盘操作的终端工具里，提供了类似IDE的调试体验，适合在远程服务器上进行调试。集成到代码中的方式与ipdb相同，不再赘述。
...
为此，\index{breakpoint}PEP 553提出了\inlinepython{breakpoint()}函数，并于Python 3.7中正式引入。相较于传统方法，\inlinepython{breakpoint()}不仅语法简洁，更具备高度可配置性，能够优雅地解决上述问题，是现代Python调试的首选方式。


\heading{基本用法}

\inlinepython{breakpoint()}的使用方式极其简单，直接将原有的\inlinepython{pdb.set\_trace()}替换为\inlinepython{breakpoint()}即可。例如，将前面\inlinefile{pdb\_demo.py}文件中的断点设置行更改为\inlinepython{breakpoint()}，程序执行到该行时会自动暂停并进入调试模式，无需任何额外的导入包语句。
...
\index{DAP}Python生态中，\index{debugpy}debugpy库是实现远程调试的官方推荐工具。它实现了Debug Adapter Protocol (DAP)，是VS Code Python扩展的调试后端，同时也被其他主流IDE广泛支持。

\heading{远程调试的组成}

\index{远程调试}远程调试通常涉及两个服务器与客户端两个部分。其中，调试服务器 (Debug Server)运行在需要被调试的Python进程（即远程端）中，它监听一个网络端口，等待调试客户端连接，并执行其下发的调试指令，如设置断点、单步执行等。调试客户端 (Debug Client)运行在开发者的本地IDE中。它连接到调试服务器，将开发者的调试操作（点击断点、查看变量）转换为操作指令发送给服务器，并展示服务器返回的程序状态。
...
\index{指标}在可观测性体系中，指标（Metrics）是数值化的系统状态数据，用于回答``系统运行得如何''这一问题。与记录离散事件的日志不同，指标以连续数值形式存在，更适合实时监控（如当前系统是否过载）和趋势分析（如近1小时请求量变化趋势）。

\subsection{Prometheus简介}

\index{Prometheus}Prometheus是一款开源的由社区独立维护的系统监控和警报工具包，最初由SoundCloud开发，现已成为云原生计算基金会（CNCF）的毕业项目。它采用拉取（pull）模式从配置的目标中收集指标数据，并支持多维度数据模型和强大的查询语言PromQL，能够灵活地聚合和查询监控数据。

\begin{figure}[htbp!]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/prometheus_architecture.pdf}
    \caption{Prometheus官网提供的架构示意图}
    \label{fig:prometheus:architecture}
\end{figure}

Prometheus的整体架构如图\ref{fig:prometheus:architecture}所示。其中，开发者所关心的部件有：

\begin{itemize}
    \item 数据采集：通过HTTP端点定期抓取目标应用暴露的指标数据。
    \item 时间序列数据库：高效存储采集到的指标数据，支持按时间序列组织。
    \item 查询与告警：通过PromQL进行数据查询和聚合，并可配置灵活的告警规则。
    \item 可视化：内置表达式浏览器，也可与Grafana等可视化工具集成。
\end{itemize}

Prometheus的生态系统中，应用通过暴露标准的HTTP端点（通常为\inlineurl{/metrics}）来提供指标数据，这些端点返回纯文本格式的指标，供Prometheus服务器抓取。这种设计使得各种语言的应用都能轻松集成监控能力。

\subsection{Prometheus-client库介绍}

Python生态中，\index{prometheus-client}prometheus-client库是实现与Prometheus集成的核心工具。它提供了简洁的API，让Python应用能够定义和更新符合Prometheus规范的指标，并将这些指标通过HTTP端点暴露出去。

该库的主要功能包括：
\begin{itemize}
    \item 指标定义：支持Counter、Gauge、Histogram等Prometheus标准指标类型。
    \item 指标暴露：自动创建HTTP端点（默认为\inlineurl{/metrics}），以Prometheus规定的文本格式返回当前指标数据。
    \item 与Web框架集成：可轻松与FastAPI、Flask等主流Web框架集成。
    \item 多进程支持：提供了在多进程环境（如Gunicorn）下收集指标的能力。
\end{itemize}

使用prometheus-client的基本流程是：在应用中定义需要的指标，在业务代码中更新这些指标，然后通过HTTP端点暴露指标数据。Prometheus服务器会定期访问这些端点抓取指标数据，进而实现监控面板的创建和异常告警。


\subsection{监控指标的常见类型}

Prometheus提供的指标类型主要有如下四种：

\begin{itemize}
    \item 计数器（Counter）：\index{计数器}只增不减的累计值，适用于记录请求总数、错误次数等。在Python中使用\inlinepython{Counter}类定义。Prometheus中常用它计算速率，如每秒请求数。
    \item 仪表盘（Gauge）：\index{仪表盘}可增可减的瞬时值，适用于记录CPU使用率、内存占用、活跃连接数等实时状态。在Python中使\inlinepython{Gauge}类定义。Prometheus中可直接查询其当前值或分析变化趋势。
    \item 直方图（Histogram）：\index{直方图}对观测值进行采样并统计分布，适用于记录请求延迟、响应大小等需要分析分布的指标。在Python中使用\inlinepython{Histogram}类定义。Prometheus中可通过它计算分位数（如95\%的请求延迟）。
    \item 摘要（Summary）：\index{摘要}与直方图类似，但直接在客户端计算分位数。在Python中使用\inlinepython{Summary}类定义。适用于需要预先计算分位数并直接暴露的场景，与直方图的选择需根据具体需求权衡。
\end{itemize}

这些指标类型为监控系统提供了丰富的数据采集能力，开发者可以根据具体监控需求选择合适的指标类型。

\subsection{监控指标数据的集成示例}

以下示例展示如何在FastAPI应用中定义和暴露Prometheus指标，如歌配置Prometheus捕获数据并进行查看。

\heading{在Python应用中集成Prometheus指标}
...
\heading{设置Prometheus的配置文件}

创建Prometheus配置文件\inlinefile{prometheus.yml}，指定对应用暴露的指标进行抓取。
...
\heading{利用Docker启动Prometheus}

Prometheus在启动时，会读取\inlinefile{/etc/prometheus/prometheus.yml}文件作为配置文件使用。此处采用Docker，将容器外的配置文件挂载到容器并启动Prometheus：
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/prometheus_ui.jpg}
    \caption{利用Prometheus UI查看http\_requests\_total的请求量变化 \label{fig:prometheus:ui}}
\end{figure}
...
\begin{enumerate}
    ...
    \item 验证联动效果：不同时间多次访问\inlineurl{http://localhost:5000/api/data}，在Prometheus UI中查看指标变化曲线，如图\ref{fig:prometheus:ui}所示。
\end{enumerate}

\subsection{监控集成实践建议}

在Python应用中集成指标监控时，遵循一些关键实践原则能确保监控数据的质量和系统的可维护性。开发者应在应用启动时统一定义指标，避免重复创建导致的混乱，并合理使用标签（如按接口端点、部署环境分类）以便在Prometheus中进行多维度聚合分析。定期评估和优化指标定义、在开发测试环境提前接入监控以早期发现问题，以及确保\inlineurl{/metrics}端点访问安全（如限制为内网或添加认证），也是构建有效监控体系的重要环节。
...
随着微服务、云原生架构的普及，单一请求往往需要在多个服务之间流转，传统的日志和指标难以完整还原请求的完整执行路径。\index{分布式追踪}分布式追踪（distributed tracing）应运而生，它通过唯一的Trace ID 将跨越服务边界的多个操作串联起来，形成完整的调用链，从而回答``一个请求是如何被处理的''这一关键问题。


\subsection{OpenTelemetry：分布式追踪的事实标准}

\index{OpenTelemetry}OpenTelemetry是一套跨语言、跨平台的遥测数据采集标准，支持日志、指标与追踪三大可观测性支柱。作为可观测性框架和工具包，OpenTelemetry专注于遥测数据的生成、采集、管理和导出，对供应商和工具保持中立，可以与各种可观测性后端（如Prometheus、Jaeger）以及商业产品无缝集成。

\heading{OpenTelemetry的核心概念}

\begin{itemize}
    \item \variable{Span}：代表一个操作单元，如一次函数调用、一次RPC请求等，并可设置由键值对构成的属性以方便查询和过滤；
    \item \variable{Trace}：由多个\variable{Span}组成的完整请求链路；
    \item \variable{Trace ID}：全局唯一的追踪标识，贯穿整个请求链路；
    \item \variable{Span Context}：用于在服务间传递追踪上下文，包含\variable{Trace ID}、\variable{Span ID}、采样标志等。
\end{itemize}

\heading{安装OpenTelemetry依赖}

在Python应用中使用OpenTelemetry，需要安装相应的SDK包。根据不同的导出需求，可选择安装不同的导出器：

\begin{minted}{bash}
# 基础SDK包（必需）
uv add opentelemetry-sdk

# Jaeger导出器（可选，用于对接Jaeger后端）
uv add opentelemetry-exporter-jaeger-thrift
\end{minted}

\heading{在Python应用中集成OpenTelemetry}

以下示例展示如何在Python应用中集成OpenTelemetry进行调用链追踪：

\begin{minted}{python}
# file: src/fxb/ch12/tracing_demo.py
"""
...
上面示例引入了JaegerExporter， 它将Span数据发送给Jaeger追踪后端。因此在运行之前，需要进一步安装Jaeger追踪后端进行对接。

\subsection{OpenTelemetry对接Jaeger}

在生产环境中，追踪数据通常需要发送到专业的追踪后端进行存储与查询。\index{Jaeger}Jaeger是一个开源的端到端分布式追踪系统，由Uber开发并开源。

\heading{使用Docker快速启动Jaeger}

最简单的Jaeger启动方式是使用Docker运行其镜像：

\begin{minted}{bash}
docker pull jaegertracing/jaeger
docker run --rm --name jaeger \
  -p 16686:16686 \    # Jaeger UI Web界面
  -p 4317:4317 \      # OTLP gRPC端口（OpenTelemetry协议）
  -p 4318:4318 \      # OTLP HTTP端口
  -p 5778:5778 \      # 配置端口
  -p 6831:6831/udp \  # Jaeger原生UDP端口（Thrift协议）
  -p 9411:9411 \      # Zipkin兼容端口
  jaegertracing/jaeger
\end{minted}

此命令运行Jaeger的All-in-one配置，将收集器和查询组件组合在单个进程中，并使用临时的内存存储来保存追踪数据，无需手动配置Jaeger组件依赖。上面容器端口的映射用途见表\ref{tab:jaeger-ports}。

\begin{table}[htbp!]
  \centering
  \small
  \caption{Jaeger容器端口映射用途说明}
  \begin{tabular}{llp{8cm}}
    \toprule
    映射配置 & 协议 & 用途说明 \\
    \midrule
    -p 16686:16686 & TCP（默认） & Jaeger的UI可视化界面端口，主机访问 \texttt{http://主机IP:16686} 即可打开Jaeger查询界面，查看追踪链路、服务依赖等 \\
    -p 4317:4317 & TCP & OTLP gRPC 协议的追踪数据接收端口（现代微服务常用，支持高性能的二进制传输），应用可通过该端口上报追踪数据 \\
    -p 4318:4318 & TCP & OTLP HTTP 协议的追踪数据接收端口（兼容性更强，基于HTTP/json传输，便于调试） \\
    -p 5778:5778 & TCP & Jaeger的配置查询端口，用于服务发现和采样策略配置的查询（兼容旧版Jaeger客户端） \\
    -p 6831:6831/udp & UDP & Jaeger Thrift 协议的UDP数据接收端口（旧版客户端常用，轻量级无连接传输） \\
    -p 9411:9411 & TCP & Zipkin 兼容端口，用于接收Zipkin格式的追踪数据（实现与Zipkin系统的兼容，平滑迁移） \\
    \bottomrule 
  \end{tabular}
\label{tab:jaeger-ports}
\end{table}

容器启动后，可以通过 \url{http://localhost:16686/}访问Jaeger的Web管理界面。

\heading{运行Python应用生成追踪数据}

现在执行\inlinefile{tracing\_demo.py}，生成追踪数据，这些数据一方面会通过控制台导出器在终端中输出，另一方面则通过JaegerExporter将数据发送到Jaeger后端。

\begin{figure}[htbp!]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/jaeger.jpg}
    \caption{订单追踪数据示例}
    \label{fig:jaeger:demo}
\end{figure}

进一步，打开\url{http://localhost:16686/}，查看追踪数据，结果如图\ref{fig:jaeger:demo}所示。

\subsection{分布式追踪实践建议}

分布式追踪的核心价值在于实现全链路可视化，能够清晰展示请求在复杂系统中的完整流转路径。通过追踪数据，开发者可以精准识别跨服务调用的性能瓶颈、快速定位故障根因，并有效梳理服务间的依赖关系，从而优化系统性能并提升问题诊断效率。

在实际项目中使用分布式追踪时，建议遵循以下实践原则：首先，应在项目早期设计追踪埋点，避免后期改造成本过高；其次，根据业务重要性设置合理的采样率，避免全量追踪带来的性能与存储压力；同时，确保Trace ID在HTTP头、消息队列等跨进程通信中正确传递；此外，通过在日志中关联Trace ID，便于在追踪与日志间跳转分析；最后，根据团队技术栈与运维能力，选择适合的后端平台进行追踪数据的存储与查询。

分布式追踪不应孤立使用，而应与日志、指标协同构建完整的可观测性体系。通过在日志中关联Trace ID、结合追踪数据计算性能指标，并在统一的可观测性仪表板中整合展示，可以实现从宏观指标到微观日志的全链路分析能力，为系统运维和问题排查提供更全面的视角。

分布式追踪是现代可观测性体系的关键组成部分，OpenTelemetry作为行业标准，提供了跨语言、跨平台的统一解决方案，使开发者能够以一致的方式收集、管理和导出追踪数据。通过将分布式追踪与日志、指标相结合，可以构建全面的系统可观测性，快速定位和解决复杂分布式系统中的问题。
...
\begin{itemize}
    \item 可观测性三大支柱：日志（发生了什么）、追踪（如何发生）、指标（运行如何）相辅相成，构成了完整的遥测数据体系。
    \item 结构化日志：通过JSON等机器可读格式提升日志处理效率，structlog库提供了优雅的实现方式。
    \item 环境感知配置：不同环境应采用不同的日志级别、输出目标和采样策略。
    \item 交互式调试：PDB用于本地调试，debugpy支持远程调试，IDE集成提升调试体验。
    \item 指标监控：Prometheus提供了强大的指标采集和查询能力，是监控系统的核心。
    \item 分布式追踪：OpenTelemetry实现了跨语言、跨服务的链路追踪，统一了遥测数据的生成标准。
\end{itemize}
