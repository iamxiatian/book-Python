...
在Python项目中，测试文件通常统一放置在项目根目录下的 \inlinefile{tests/} 目录中。这种集中管理的方式便于开发工具和CI/CD流程的自动发现和处理。根据项目规模和复杂度，可以选择以下两种基本结构：

\circled{1} 扁平结构：所有测试文件直接放在 \inlinefile{tests/} 目录下。
...
\circled{2} 镜像结构：测试文件目录结构完全镜像源代码结构。
...
\begin{itemize}
    \item 小型项目：可直接使用扁平结构，简单直观。
    \item 中大型项目：建议采用镜像结构，便于维护和避免命名冲突。
\end{itemize}

\heading{大型项目的分层目录组织结构}

测试领域有一个测试金字塔（Test Pyramid）理念（详细内容见第\ref{subsec:test-pyramid}节），为匹配该理念，还可以按照如下方式建立分层子目录，对测试相关文件进行分类组织：
