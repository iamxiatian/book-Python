...
在实践中，可以将这两个原则结合起来应用。首先通过高内聚设计创建职责清晰的模块，然后通过低耦合策略管理模块间的关系。值得注意的是，模块化设计不是一次性的任务，而是需要在整个项目生命周期中持续关注和改进的过程。随着需求的变化和系统的演进，应该定期审视模块的职责边界和依赖关系，确保设计始终符合当前的需求和约束条件。
...
现代Python项目推荐采用统一的配置文件管理策略，将所有工程配置和元数据文件集中放置在项目根目录。这种集中化管理理念源于Unix哲学中的约定优于配置原则，便于开发工具和CI/CD流程的自动发现和处理。
...
测试文件应该统一放置在根目录的\inlinefile{tests/}目录中，遵循\inlinefile{test\_*.py}的命名规范，便于测试框架自动发现。合理的测试目录结构应该区分单元测试和集成测试，为不同类型的测试提供独立的运行环境。此外，如果你的项目比较简单，也可以直接将测试文件放在\inlinefile{tests/}目录下。

\subsection{文档的持续维护策略}

正如Python之禅中提到的``可读性很重要''，专业项目的文档管理应遵循与代码同等重要的原则，文档应该与代码同步维护。
...
初学者在构建 Python 项目时，常会踩中``过早优化结构''的坑：还没写几行代码就急着把目录拆成深层级的 src、core、utils，结果每次新建文件都要在多层文件夹里来回跳转，反而拖慢开发节奏。最简单的办法是只用一级或两级目录把当前功能放下即可，等真正出现文件太多不好查找的情况再进行拆分。

导入路径混乱也是初学者常遇到的问题，一会儿\inlinepython{from ..utils import foo}，一会儿\inlinepython{from src.utils import foo}，路径提示全靠开发工具猜，换台机器或跑个 pytest就报ImportError。解决这一问题的简答办法是采用\inlinefile{pyproject.toml}和uv等工具，把包安装到虚拟环境，采用\inlinepython{from my\_project.utils import foo}这种形式的绝对导入方式。
...
\textbf{关键要点回顾：}
\begin{itemize}
    \item {渐进式演进}：项目结构应随项目复杂度逐步演进
    \item {工具集成}：现代工具为最佳实践提供了良好支持
    \item {团队协作}：统一的规范比完美的结构更重要
\end{itemize}
