...
现代软件工程已将测试提升至核心地位——测试不仅是发现缺陷的工具，更是驱动设计、规范行为、构建信心的系统工程方法。要理解测试的重要性，我们可以借鉴其他工程领域的实践。例如，在桥梁建设中，工程师不仅会在完工后进行载荷测试，更会在设计阶段通过模拟计算验证结构强度。这种验证前置的理念同样适用于软件开发。
...
这种循环的核心理念在于，测试不是事后验证，而是设计工具。通过先编写测试，开发者被迫思考代码应该如何被使用，这常常会带来更清晰、更用户友好的API设计。例如，在实现一个字符串反转功能时，TDD的循环可如下展开：
...
TDD不仅适用于单元测试，也可用于集成测试与验收测试。其关键不是"先写测试"，而是让测试引导你写代码。每一次红绿重构循环，都是对系统设计的一次演进。这种迭代式的开发方式使得代码质量在开发过程中持续提升，而不是事后补救。
...
\section{测试基础：核心概念与基本元素}
\label{sec:testing-fundamentals}

无论使用何种测试框架，测试的核心都由断言（Assertion）和测试用例（Test Case）构成。理解这两个基本元素是编写任何测试的前提。这些基础概念构成了测试思维的基石，掌握它们后，无论面对多么复杂的测试框架，都能迅速理解其工作原理。
...
\heading{为什么使用断言而不是print语句？}

初学者常常会问：为什么不能直接使用print语句来检查代码是否正确？比较以下两种方式：
...
这种结构的好处是：
\begin{itemize}
    \item 清晰分离：每个阶段职责明确，代码易于理解
    \item 易于调试：当测试失败时，可以快速定位问题出现在哪个阶段
    \item 可维护性高：修改测试时，只需关注相关阶段
\end{itemize}
...
\subsection{从零开始编写一个测试}

让我们通过一个完整的例子，看看如何从零开始编写测试：

\begin{minted}{python}
# 第一步：编写要测试的代码
# src/fxb/ch10/calculator.py
def divide(a, b):
    """安全的除法函数，处理除零错误"""
    if b == 0:
...
# 第二步：编写测试
# tests/ch10/test_calculator.py
import pytest
from fxb.ch10.calculator import divide # 导入被测试的模块
...
\subsection{测试思维的核心要点}

理解断言和测试用例后，我们可以归纳出测试思维的核心要点：

\begin{itemize}
    \item 测试即验证：测试的本质是验证代码行为是否符合预期
    \item 断言是核心：测试使用断言来自动化验证过程
    \item 结构要清晰：测试遵循``准备-执行-验证''的三段式结构
    \item 命名要明确：测试名称应该清晰描述测试目的
    \item 覆盖要全面：不仅要测试正常情况，还要考虑边界条件和异常情况
\end{itemize}
...
伴随着unittest的使用，Python社区在早年还开发了nose框架，旨在提供更简洁的测试发现和运行体验。nose通过插件机制增强了测试框架的可扩展性，允许使用简单函数作为测试用例：
...
随着nose项目在2015年后逐渐停止活跃维护，其继任者nose2随之诞生。nose2扩展了unittest以改善测试体验，但它与nose并不完全兼容，且主要作为unittest的增强版而非全新框架\footnote{\url{https://docs.nose2.io/}}。尽管nose2仍在维护，但其社区规模和插件生态相对有限，大部分项目已转向更加现代化和功能丰富的测试框架。
...
如果测试文件中只有测试用例，没有指定入口函数，则可以通过unittest模块执行测试：
\begin{minted}{bash}
# 利用uv命令运行
...
\begin{itemize}
    \item 装饰器路径指定： \inlinepython{@patch("模块路径.目标对象")}装饰器是\inlinepython{patch()}最常用的形式，其参数应传入被测代码中实际引用目标对象的完整路径。如示例中的\inlinepython{@patch("requests.get")}，表示模拟requests的get请求函数。
...
虽然Python标准库的unittest.mock功能完整，但在使用pytest时，直接使用\inlinepython{patch()}装饰器仍会存在不便，例如：装饰器嵌套层级深、模拟对象参数顺序易混淆、手动管理上下文繁琐等。pytest-mock是pytest的官方扩展插件\footnote{\url{https://pytest-mock.readthedocs.io/}}，它对unittest.mock进行了轻量级封装，提供了更简洁、更符合pytest风格的模拟方式，核心优势是无需手动导入patch、无需处理装饰器嵌套、通过fixture统一管理模拟对象。
...
pytest-mock将mock功能集成到pytest的fixture系统中，为pytest框架提供了更加简洁优雅的模拟支持，相较于直接使用Python内置的unittest.mock模块，能显著提升测试代码的清晰度和编写效率。

pytest-mock最直接的改进是在完全兼容unittest.mock基础上，提供了开箱即用的mocker装置。如前面的示例所示，你无需在测试文件中手动导入Mock、patch等类，只需在测试函数签名中声明mocker参数，即可直接使用。这移除了不必要的导入语句，让测试函数的焦点完全保持在测试逻辑本身。
