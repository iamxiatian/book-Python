...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}>{\centering\arraybackslash}p{2.5cm} p{3cm} p{3cm} p{3cm}@{}}
    \toprule
    特性 & \makecell{传统工具链\\(2010-2020)} & 过渡期工具 & \makecell{现代工具链\\(2020+)}  \\
    \midrule
...
将上述代码保存为 \inlinefile{src/demo_black.py} 并执行：
...
Black能够从\inlinefile{pyproject.toml}文件中读取命令行选项的项目专用默认值。这对于为项目指定自定义的\mintinline{text}{--include}和\mintinline{text}{--exclude}模式特别有用。
...
Ruff在C901规则中实现了圈复杂度检查。通过在\mintinline{toml}{[tool.ruff.lint]}的select中启用C901，或者设置为包含C901的父集合规则，如"C9"、"ALL"，即可启用圈复杂度检查。同时，通过在\mintinline{toml}{[tool.ruff.lint.mccabe]}中设置max-complexity参数，可改变圈复杂度的警示阈值。
...
\begin{itemize}
    \item 采用Ruff实现自动化的代码格式化与检查，获得现代化工具的性能优势；
    \item 在 \inlinefile{pyproject.toml} 中集中管理所有工具配置，简化项目维护；
    \item 将Pre-Commit 检查集成到团队工作流，建立代码质量文化；
    \item 定期进行复杂度分析和技术债评估，持续改进代码结构；
\end{itemize}

\textbf{进阶思考：}即使代码通过了所有静态检查，也不意味着它能稳定运行或具有良好的架构设计。下一阶段，我们将转向软件设计原则和架构模式，探讨如何通过SOLID原则、设计模式和清晰的抽象边界，构建可扩展的软件系统。